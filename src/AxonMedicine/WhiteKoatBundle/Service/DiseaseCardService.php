...
class DiseaseCardService extends RelationshipService
{
    ...
    {
        $query = $this->em->createQuery('select a from AxonMedicine\WhiteKoatBundle\Entity\DiseaseCardView a inner join a.diseasename b order by b.name ');
    ...
    {
        $query = $this->em->createQuery('select a from AxonMedicine\WhiteKoatBundle\Entity\DiseaseCardView a inner join a.diseasename b where a.diseasename=?1 order by b.name ');
